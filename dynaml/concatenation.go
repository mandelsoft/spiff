...
func (e ConcatenationExpr) Evaluate(binding Binding, locally bool) (interface{}, EvaluationInfo, bool) {
	...
	if !aok {
		...
		if !aok {
			return info.Error("illegal type '%s' for first concatenation argument", expression_type(a))
		}
		switch bmap := b.(type) {
		...
		case nil:
			return a, info, true
		default:
			return info.Error("type '%s' cannot be concatenated with type '%s'", expression_type(b), expression_type(a))
		}
	} else {
		switch b.(type) {
		case []yaml.Node:
