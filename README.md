...
The function `hash` generates several kinds of hashes for the given data.
...
if `data` evaluates to a string, the hash is generated for the byte representation 
of the string.

`md5`hashes can still be generated by the deprecated function `md5(string)`.
...
Alternatively, any data structure can be passed. For any non-string type,
a [Canonical JSON Scheme](https://www.rfc-editor.org/rfc/rfc8785.html) 
of the data is hashed.
...
The hash type identified may be preceded with some data type indicator:
- `*`: enforces a JCS interpretation of the data, even if it a simple string type
- `#`: expects binary data represented as base64-encoded string.

The data type indicator can be used without a hash type identifier, also.
In this case the default hash type is used.
...
The function `asjson` takes an optional second argument. It is evaluated as boolean. If it evaluates to `true`, the generated _json_ is compliant with [RFC 8785: JSON Canonicalization Scheme (JCS)
...
be used to pass write options. It basically behaves
